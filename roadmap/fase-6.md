...
- [x] `--filter charge` retorna só `charge` (não `refund`, não `pending`)
- [x] `--filter find` retorna `findActive`
- [x] Filtro sem match → `methods` vazio ou mensagem clara (comportamento definido e consistente)
- [x] Funciona com `--text` e JSON
